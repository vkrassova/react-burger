...
const BurgerIngredients: React.FC = () => {
  ...
  const scroll = () => {
    const tabsBottom = Math.abs(
      Math.round(getRect(tabsRef?.current).bottom - PADDING_BOTTOM)
    );
    const mainTop = Math.abs(
      Math.round(getRect(mainRef?.current).top - tabsBottom)
    );
    const saucesTop = Math.abs(
      Math.round(getRect(sauceRef?.current).top - tabsBottom)
    );
    const bunTop = Math.abs(
      Math.round(getRect(bunRef?.current).top - tabsBottom)
    );
